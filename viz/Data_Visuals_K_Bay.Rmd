...
pdo_plot <- ggplot(data = pdo_mon, aes(x = Year, y = PDO)) + geom_col() + 
            geom_hline(yintercept = 0, linewidth = 1.5) + ylab("PDO Index")

  
#
pdo_plot2 <- ggplot(data = pdo_mon_adj, aes(x = Yr_Mn, y = PDO)) + 
             geom_col(aes(fill = Sign), show.legend = FALSE) + xlab("Year") +            #
             geom_hline(yintercept = 0, linewidth = 1) + ylab("PDO Index") + 
...
AllData_clean2 <- AllData_clean %>% 
                  ...
                  mutate(MASTO_PAP = str_replace_all(MASTO_PAP, ",",""),
                         SIPHONARIA = gsub("[*]", "", SIPHONARIA)) %>%
                  mutate(across(c(37, 49), as.numeric)) %>% # converts select columns
...
PerCov_long <- AllData_clean2 %>%
               gather(TAXA, PER_COV_OR_COUNT, -YEAR, -QUAD#, -TRIPLET, -TREATMENT
                      )
...
PerCov_anom <- PerCov_long %>%
               ...
               mutate(across(c(3), as.numeric)) %>% # converts select columns
               group_by(TAXA) %>%
               mutate(TAXA_Mean = mean(PER_COV_OR_COUNT),
                      ANOMALY = PER_COV_OR_COUNT - TAXA_Mean) %>%
               ungroup() %>%
               group_by(TAXA, YEAR) %>%
               mutate(YEAR_Mean = mean(PER_COV_OR_COUNT),
                             YEAR_Anom = mean(YEAR_Mean - TAXA_Mean),
                             YEAR_Anom_Scale = YEAR_Anom/100,
                             YEAR_Anom_Scale2 = YEAR_Anom/10) %>%
               ungroup() %>%
               arrange(YEAR, TAXA#, TREATMENT
                       )
...
explore_plots <- function(y_var){
                 ...
                 p <- ggplot(data = AllData_clean2, aes(x = as.factor(YEAR), y = !!y_var#, fill = TREATMENT
                                                        )) +
 ...
 # ggplot(data = AllData_clean2, aes(x = as.factor(YEAR), y = MASTO_PAP#, fill=TREATMENT 
 #                                   )) +
...
# all taxa
# AllData_clean2[,c(3:52)]
# 
# examine more
# var_names <- names(AllData_clean2[,c(6, 11, 29, 37, 49)])

# taxa of interest based on abundance - numbers of column numbers
# c(8, 10, 16, 18, 22, 30, 31, 34, 39, 41, 42, 48, 51)

# make vector of variables
var_names <- names(AllData_clean2[,c(8, 10, 16, 18, 22, 30, 31, 34, 37, 39, 41, 42, 48, 49, 51)]) 

# explore_plots(var_names[4])
...
myt_sub <- unique(PerCov_anom[PerCov_anom$TAXA == "MYTILUS",c(1,2,8,9)])
myt_sub2 <- myt_sub %>% 
            mutate(YEAR_Mon = paste(YEAR,"-06", sep = "")) %>% 
            mutate(across(c(1), as.factor)) %>% # converts select columns
            mutate(myt_sub, YEAR_Anom_Scale3 = YEAR_Anom_Scale2 * 10000)
...
###
fuc_sub <- unique(PerCov_anom[PerCov_anom$TAXA == "FUCUS_PERCOV_TOTAL",c(1,2,8,9)])
...
###
brn_sub <- unique(PerCov_anom[PerCov_anom$TAXA == "BARNACLES",c(1,2,8,9)])
...
###
scu_sub <- unique(PerCov_anom[PerCov_anom$TAXA == "L.SCUTULATA",c(1,2,8,9)])
...
###
sit_sub <- unique(PerCov_anom[PerCov_anom$TAXA == "L.SITKANA",c(1,2,8,9)])
...
spr2 <- ggplot(data=spring_a_temp, aes(x=YrMnDy, y=month_Anom)) + 
        geom_col(aes(fill=month_Sign)) + ylab("Anomaly") + xlab("Year") + 
        theme_bw() + geom_hline(yintercept=0, linewidth=1) + 
        ...
        scale_x_discrete(breaks=c(#"1999-03-01", "2000-03-01", "2001-03-01", 
                                  "2002-03-01",
...
sum2 <- ggplot(data=summer_a_temp, aes(x=YrMnDy, y=month_Anom)) + 
        ...
        scale_fill_manual(values=c("A"="#f88379", "B"="#00BFC4"),
                          name="",
                          labels=c("positive temp", "negative temp")) +
        scale_x_discrete(breaks=c(#"1999-06-01", "2000-06-01", "2001-06-01", 
                                  "2002-06-01",
...
fal2 <- ggplot(data=fall_a_temp, aes(x=YrMnDy, y=month_Anom)) + 
        ...
        scale_fill_manual(values=c("A"="#f88379", "B"="#00BFC4"),
                          name="",
                          labels=c("positive temp", "negative temp")) +
        scale_x_discrete(breaks=c(#"1999-09-01", "2000-09-01", "2001-09-01", 
                                  "2002-09-01",
                                  ...
                                  "2019-09-01", "2020-09-01", "2021-09-01", "2022-09-01"),
                         labels=c(#"1999", "2000", "2001", 
                                  "2002", "2003", "2004", "2005", 
                                  "2006", "2007", "2008", "2009", "2010", "2011", "2012",
                                  "2013", "2014","2015", "2016", "2017", "2018", "2019", 
...
win2 <- ggplot(data=winter_a_temp, aes(x=YrMnDy, y=month_Anom)) + 
        ...
        scale_fill_manual(values=c("A"="#f88379", "B"="#00BFC4"),
                          name="",
                          labels=c("positive temp", "negative temp")) +
        scale_x_discrete(breaks=c(#"1999-12-01", "2000-12-01", "2001-12-01", 
                                  "2002-12-01",
...
# NMDS output
percov_mds <- readRDS(here("./data_clean/percov_mds.rds"))
percov_mds2 <- readRDS(here("./data_clean/percov_mds2.rds"))
percov_mds3 <- readRDS(here("./data_clean/percov_mds3.rds"))
percov_mds4 <- readRDS(here("./data_clean/percov_mds4.rds"))
percov_mds5 <- readRDS(here("./data_clean/percov_mds5.rds"))

# Permanova output
percov_perm <- readRDS(here("./data_clean/percov_perm.rds"))
percov_perm2 <- readRDS(here("./data_clean/percov_perm2.rds")) 
percov_perm3 <- readRDS(here("./data_clean/percov_perm3.rds"))
percov_perm4 <- readRDS(here("./data_clean/percov_perm4.rds"))
percov_perm5 <- readRDS(here("./data_clean/percov_perm5.rds"))
...
ord8 <- ggplot(data=NMDS_ord5, aes(x=NMDS1, y=NMDS2)) +
        ...
        geom_segment(data=sp_vecs, aes(x=0, xend=MDS1, y=0, yend=MDS2), color="grey", 
                     lidewidth = 1.25, arrow=arrow(length=unit(0.35, "cm"))) +
...
ord9 <- ggplot(data=NMDS_ord5, aes(x=NMDS1, y=NMDS2)) +
        ...
        geom_segment(data=sp_vecs, aes(x=0, xend=MDS1, y=0, yend=MDS2), color="grey", 
                     linewidth = 1.25, arrow=arrow(length=unit(0.35, "cm"))) +
        geom_text(data=sp_vecs, aes(x=MDS1, y=MDS2, label=Species), size=4) +
        theme(panel.grid.major=element_blank(),
              panel.grid.minor=element_blank())
ord9
...
ord10 <- ggplot(data=NMDS_ord5, aes(x=NMDS1, y=NMDS2)) +
         geom_point(size=4, aes(color=YEAR)) + theme_bw() + 
         geom_segment(data=sp_vecs2, aes(x=0, xend=MDS1, y=0, yend=MDS2), color="grey",
                      linewidth = 1.25, arrow=arrow(length=unit(0.35, "cm"))) +
