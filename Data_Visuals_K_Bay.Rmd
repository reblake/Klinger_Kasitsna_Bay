...
PerCov_anom <- PerCov_long %>%
               ...
               group_by(Taxa) %>%
               mutate(Taxa_Mean = mean(Percent_Cover),
                      Taxa_Anom = Percent_Cover-Taxa_Mean) %>%
               ungroup() %>% 
               filter(Treatment == "01") %>%
               group_by(Taxa, Year) %>%
               mutate(Year_Mean = mean(Percent_Cover),
                      Year_Anom = mean(Year_Mean-Taxa_Mean),
                      Year_Anom_Scale = Year_Anom/100) %>%
               ungroup() %>%
               arrange(Year, Block, Treatment, Taxa)
...
anom_pdo_muss <- pdo_plot2 + 
                 geom_col(data=PerCov_anom[PerCov_anom$Taxa == "Mytilus",], aes(x=Year,y=Year_Anom_Scale),
                          width=2) + 
...
anom_pdo_fucu <- pdo_plot2 + 
                 geom_col(data=PerCov_anom[PerCov_anom$Taxa == "FUCUS_TOTAL",], aes(x=Year,y=Year_Anom_Scale),
                          width=2) + 
                 ylab("Anomaly") + theme_bw() + ylim(-4,4) +
                 theme(panel.grid.major=element_blank(),
                       panel.grid.minor=element_blank(),
...
anom_pdo_barn <- pdo_plot2 + 
                 geom_col(data=PerCov_anom[PerCov_anom$Taxa == "Barnacles",], aes(x=Year,y=Year_Anom_Scale),
                          width=2) + 
                 ylab("Anomaly") + theme_bw() + ylim(-4,4) +
                 theme(panel.grid.major=element_blank(),
                       panel.grid.minor=element_blank(),
