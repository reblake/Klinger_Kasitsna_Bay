...
explore_plots <- function(y_var){
                 ...
                 p <- ggplot(data=AllData_clean2, aes_string(x="YEAR", y=y_var, fill="TREATMENT1")) + 
                      ...
                      ylab(paste(toupper(substring(y_var, 1, 1)), 
                                 tolower(substring(y_var, 2)), 
                                 " percent cover", sep = "")) + 
                      ...
                      theme(panel.grid=element_blank(),
                            ...
                            axis.text.x=element_text(size=13, angle=35, hjust=0.9),
...
explore_plot2 <- function(y_var){
                 ...
                 q <- ggplot(data=AllData_clean2, aes_string(x="YEAR", y=y_var, fill="TREATMENT1")) + 
                      geom_boxplot() + theme(legend.position="top", legend.title=element_blank()) + 
                      theme_bw() + xlab("Year") + 
                      ylab(expression(paste("Number of ", italic(y_label)))) + 
                      scale_fill_manual(breaks=c("CONTROL","SCRAPE99","SCRAPE00"),
                                        labels=c("Control","Scrape 1999","Scrape 2000"),
                                        values=c("#76eec6","#63b8ff","#ff6a6a")) + 
                      theme(panel.grid=element_blank(),
                 ...
                 # rename some columns for plot labeling purposes
                # names(AllData_clean2)[names(AllData_clean2) == 'FUCUS_PERCOV_TOTAL'] <- 'FUCUS'
                # names(AllData_clean2)[names(AllData_clean2) == 'FUCUS_SPORELINGS_PERCOV'] <- '`FUCUS SPORELINGS`'
                # names(AllData_clean2)[names(AllData_clean2) == 'BARNACLES'] <- 'BARNACLE'   
                # names(AllData_clean2)[names(AllData_clean2) == 'BARNACLE_SPAT'] <- '`BARNACLE SPAT`' 
...
NMDS_ord5 <- data.frame(NMDS1 = percov_mds5$points[,1], NMDS2 = percov_mds5$points[,2],
                        Year = all_treats2$Year, Plot = all_treats2$plot,
                        ATmp_Anom_Sign = all_treats2$ATmp_Sign,
                        FWD_annSign = all_treats2$FWD_Sign, PDOSign = all_treats2$PDO_Sign)

# make convex hulls for each plot across all years
find_hull <- function(df) df[chull(df$NMDS1, df$NMDS2), ]
hulls_all <- ddply(NMDS_ord5, "Plot", find_hull)

# now the plot
ord6 <- ggplot(NMDS_ord5, aes(x=NMDS1, y=NMDS2, color=Plot)) +
        geom_point(size=4) + theme_bw() + 
        geom_polygon(data=hulls_all, aes(fill=Plot), alpha=0.3, size=1) +
        theme(panel.grid.major=element_blank(),
              panel.grid.minor=element_blank())
ord6
...
ord7 <- ggplot(NMDS_ord5, aes(x=NMDS1, y=NMDS2, color=Year)) +
        geom_point(size=4) + theme_bw() + 
        geom_polygon(data=hulls_all2, aes(fill=ATmp_Anom_Sign), alpha=0, size=1) +
        theme(panel.grid.major=element_blank(),
              panel.grid.minor=element_blank())
ord7
...
lmfit_tfu <- lm(ATemp_YearMn~FUCUS_TOTAL, data=PerCov_all)  # lm() fit 

cor_plot5 <- ggplot(data=PerCov_all, aes(x=FUCUS_TOTAL, y=ATemp_YearMn)) + 
             geom_point(size=4) + stat_smooth(method="lm", col="pink") + theme_bw() +
...
lmfit_fwf <- lm(mn_yr_discharge~FUCUS_TOTAL, data=PerCov_all)  # lm() fit 

cor_plot7 <- ggplot(data=PerCov_all, aes(x=FUCUS_TOTAL, y=mn_yr_discharge)) + 
             geom_point(size=4) + stat_smooth(method="lm", col="pink") + theme_bw() +
...
# arranging above plots
grid.arrange(cor_plot4, cor_plot5, cor_plot5b, cor_plot6, cor_plot7, cor_plot8, ncol=3)

```

```{r, echo=FALSE, fig.height=6, fig.width=8}
lmfit_fb <- lm(Barnacles~FUCUS_TOTAL, data=PerCov_all)  # lm() fit 

cor_plot9 <- ggplot(data=PerCov_all, aes(x=FUCUS_TOTAL, y=Barnacles)) + 
             geom_point(size=4) + stat_smooth(method="lm", col="pink") + theme_bw() +
