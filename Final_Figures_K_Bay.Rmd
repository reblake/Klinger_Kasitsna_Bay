...
explore_plot3 <- function(y_var){
                 ...
                 q <- ggplot(data=CONTData_clean, aes_string(x="YEAR", y=y_var, fill="TREATMENT1")) + 
                      geom_boxplot() + theme(legend.position="top", legend.title=element_blank()) + 
                      theme_bw() + xlab("Year") + 
                      ylab(paste("Number of ", y_label)) + 
                      scale_fill_manual(breaks=c("CONTROL","SCRAPE99","SCRAPE00"),
...
BARN_SPAT_plot <- PerCov_Plots[[1]] + ylab("Barnacle spat % cover") + theme(legend.position="none",
                                                                            axis.title.x=element_blank(),
                                                                            axis.text.x=element_blank())  
BARN_plot <- PerCov_Plots[[2]] + ylab("Barnacle % cover") + theme(legend.position="none",
                                                                  axis.title.x=element_blank(),
                                                                  axis.text.x=element_blank()) 
FUCUS_plot <- PerCov_Plots[[3]] + ylab(expression(paste(italic("Fucus"), " % cover")))  +
...
FUC_SPORE_plot <- PerCov_Plots[[4]] + ylab(expression(paste(italic("Fucus"), " sporeling % cover")))  +
                  theme(legend.position="none",
                        axis.title.x=element_blank(),
                        axis.text.x=element_blank()) 
MUSS_plot <- PerCov_Plots[[5]] + ylab(expression(paste(italic("Mytilus"), " % cover")))  +
             theme(legend.position="none",
                   axis.title.x=element_blank(),
                   axis.text.x=element_blank()) 
PTERO_plot <- PerCov_Plots[[6]] + ylab(expression(paste(italic("Pterosiphonia"), " % cover"))) +
...
SNAIL_plot <- Count_Plots[[1]] + ylab(expression(paste(italic("L. sitkana"), " abundance"))) 
LIMP_plot <- Count_Plots[[2]] + ylab(expression(paste(italic("Lottiidae"), " abundance"))) 
...
WT_plot1 <- ggplot(data=WTemp_June, aes(x=Year, y=Water_Temp_June, group=1)) + 
            ...
            ylab(expression(paste("June water temperature ( ", degree ~ C,")"))) +
            theme(panel.grid=element_blank(),
                  legend.title=element_blank(),
                  axis.text.y=element_text(size=11),
                  #axis.text.x=element_text(size=11, angle=35, hjust=0.9),
...
AT_plot1 <- ggplot(data=year_a_temp, aes(x=Year, y=ATemp_YearMn, group=1)) + 
            ...
            ylab(expression(paste("Annual air temperature ( ", degree ~ C,")"))) +
            theme(panel.grid=element_blank(),
                  legend.title=element_blank(),
                  axis.text.y=element_text(size=11),
                  axis.text.x=element_text(size=11, angle=35, hjust=0.9),
