...
PerCov_Fresh <- PerCov_PDO %>%
                ...
                dplyr::rename(year = Year) %>% 
                dplyr::filter(!(year %in% c("1999", "2000", "2001")))  # filer out year we don't have FWD for
...
PerCov_all <- PerCov_PDO %>%
              ...
              dplyr::rename(ATmp_Sign = year_Sign,
                            ...
                            FWD_Sign = Sign) #%>%
              #dplyr::filter(!Year %in% c("2019","2020","2021")) # filter out incomplete FWD years
...
# calculate PERMANOVA 
percov_perm4 <- vegan::adonis2(sp_percov4 ~ ATemp_yearMn + mn_yr_discharge,
...
# calculate PERMANOVA 
# percov_perm5 <- vegan::adonis2(sp_percov5 ~ ATemp_yearMn * mn_yr_discharge * Water_Temp_June, all_treats2, 
